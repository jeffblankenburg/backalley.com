...
export async function createCompletedGameInSupabase(
  game: Game,
  createdBy: string,
): Promise<string> {
  // 1. Insert game
  const { data: gameData, error: gameError } = await supabase
    ...
    .insert({
      status: game.status,
      starting_dealer_index: game.startingDealerIndex,
  ...
  // 2. Insert game_players
  const gamePlayers = game.playerIds.map((userId, i) => ({
    game_id: gameId,
    user_id: userId,
    seat_position: i,
  }));
  ...
  // 3. Insert rounds
  const roundInserts = game.rounds.map((r) => ({
    game_id: gameId,
    round_index: r.roundIndex,
    hand_size: r.handSize,
    trump_suit: r.trumpSuit,
    dealer_user_id: r.dealerPlayerId,
    bids_entered: r.bidsEntered,
  ...
  // 4. Insert player_rounds
  const roundIdByIndex = new Map(roundData.map((r) => [r.round_index, r.id]));
  const playerRoundInserts: {
    round_id: string;
    user_id: string;
    bid: number;
  ...
  for (const round of game.rounds) {
    const roundId = roundIdByIndex.get(round.roundIndex);
    if (!roundId) continue;
